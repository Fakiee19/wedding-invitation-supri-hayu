...
(function () {
  ...
  const WEB_APP_URL =
    "https://script.google.com/macros/s/AKfycbwyaBYS8_MzkMPHxwcjlOFmD2TBMP8kaR8meQ0PlRD3az7ltOQQq5tP6J4cHGWHQKTAkA/exec";
  ...
  function initParticles() {
    ...
    function frame() {
      ...
      particles.forEach((p) => {
        ...
        ctx.fillStyle = `rgba(114, 47, 55, ${p.a})`;
        ctx.fill();
      });
  ...
  function initPremiumScroll() {
    ...
    // 1. Float & Parallax Hero Elements
    const lights = $(".hero__lights");
    const heroMedia = $(".hero__media");
    const heroPhotos = $$(".hero__photo img");
    ...
    // 2. Cinematic Section Revealer
    $$(".section").forEach((sec) => {
      gsap.fromTo(sec,
        { opacity: 0.4, y: 40 },
        {
          opacity: 1, y: 0, duration: 1.8, ease: "power3.out",
          scrollTrigger: { trigger: sec, start: "top 95%", toggleActions: "play none none reverse" }
        }
      );
    });

    // 3. Image Reveal for Couple Cards
    $$(".couple-card__frame").forEach((frame) => {
    ...
    // 4. Elegant Text Cascade for Section Titles
    ...
    // 6. Slow Decorative Elements / Background Motion
    const grain = $(".grain");
    if (grain) {
      gsap.to(grain, {
    ...
    const storyDots = $$(".story-node__dot");
    storyDots.forEach(dot => {
      gsap.to(dot, {
        scale: 1.15,
        ease: "none",
        scrollTrigger: { trigger: dot, start: "top 80%", end: "bottom 50%", scrub: true }
      });
    });
  }
  ...
  function initGallery() {
    ...
    $$(".gallery-item").forEach((fig) => {
      fig.addEventListener("click", (e) => {
        // On touch screens, first click expands, second click opens lightbox
        if (window.matchMedia("(hover: none)").matches) {
          if (!fig.classList.contains("is-expanded")) {
            // Remove from all others
            $$(".gallery-item").forEach((el) =>
              el.classList.remove("is-expanded"),
            );
            fig.classList.add("is-expanded");
            return; // Don't open lightbox on first tap
          }
        }
    ...
    // Close expanded item if clicking outside on mobile
  ...
  function initRsvp() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (!isConfigured()) {
        alert(
          "Mohon set WEB_APP_URL di js/script.js setelah deploy Google Apps Script.",
        );
        return;
      }
  ...
  async function loadWishes(force) {
    ...
    try {
      ...
      if (!wishes.length) {
        ...
        if (ph) {
          ph.textContent =
            "Belum ada ucapan. Jadilah yang pertama melalui RSVP.";
          ph.classList.remove("is-hidden");
        }
  ...
  function runLoaderAnim() {
    ...
    tl.from(".loader__label", {
      ...
      .from(
        ".loader__line",
        { scaleX: 0, duration: 1.05, ease: "power2.inOut" },
        "-=0.25",
      )
      .from(
        ".loader__logo-wrap",
      ...
      .from(
        ".loader__names",
        { y: 16, opacity: 0, duration: 1.15, ease: "power2.out" },
        "-=0.3",
      )
      .to({}, { duration: HOLD_MS });
  }
